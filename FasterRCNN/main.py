...
def enable_cuda_memory_profiler(model):
    ...
    import sys
    import threading
    memory_profiler = myprofile.CUDAMemoryProfiler([model], filename="cuda_memory.txt")
    ...
    for epoch in range(1, 1 + options.epochs):
        ...
        if options.checkpoint_dir:
            checkpoint_file = os.path.join(options.checkpoint_dir,
                                           "checkpoint-epoch-%d-mAP-%1.1f.pth" % (epoch, mean_average_precision))
...
def predict_one(model, url, show_image, output_path):
    ...
    image_data, image_obj, _, _ = image.load_image(url=url, preprocessing=model.backbone.image_preprocessing_params,
...
def predict_all(model, split):
    ...
    for sample in iter(dataset):
        ...
        predict(model=model, image_data=sample.image_data, image=sample.image, show_image=False,
                output_path=output_path)
...
if __name__ == "__main__":
    ...
    group.add_argument("--predict-to-file", metavar="url", action="store", type=str, default="file.jpg",
    ...
    parser.add_argument("--dataset-dir", metavar="dir", action="store", default="../datasets/VOC2028",
                        help="VOC dataset directory")
    parser.add_argument("--train-split", metavar="name", action="store", default="trainval",
                        help="Dataset split to use for training")
    parser.add_argument("--eval-split", metavar="name", action="store", default="val",
                        help="Dataset split to use for evaluation")
    parser.add_argument("--cache-images", action="store_true",
                        help="Cache images during training (requires ample CPU memory)")
    parser.add_argument("--periodic-eval-samples", metavar="count", action="store", default=1000,
                        help="Number of samples to use during evaluation after each epoch")
    parser.add_argument("--checkpoint-dir", metavar="dir", action="store",
                        help="Save checkpoints after each epoch to the given directory")
    parser.add_argument("--plot", action="store_true",
                        help="Plots the average precision of each class after evaluation (use with --train or --eval)")
    ...
    parser.add_argument("--dropout", metavar="probability", type=float, action="store", default=0.0,
                        help="Dropout probability after each of the two fully-connected detector layers")
    parser.add_argument("--no-augment", action="store_true",
                        help="Disable image augmentation (random horizontal flips) during training")
    parser.add_argument("--exclude-edge-proposals", action="store_true",
                        help="Exclude proposals generated at anchors spanning image edges from being passed to detector stage")
    parser.add_argument("--dump-anchors", metavar="dir", action="store",
                        help="Render out all object anchors and ground truth boxes from the training set to a directory")
    parser.add_argument("--profile-cuda-memory", action="store_true",
                        help="Profile CUDA memory usage and write output to 'cuda_memory.txt'")
